...
def get_intraday_features(ticker, days=60, interval="5m"):
    ...
    if intraday.empty:
        print(f"No intraday {interval} data found for {ticker} in last {days} days.")
        return pd.DataFrame()
...
def short_momentum_signal(data, period=5, threshold=1.5):
    if len(data) < period:
        return 0
    mom_val = (data['Close'].iloc[-1] - data['Close'].iloc[-period]) / data['Close'].iloc[-period] * 100
...
def backtest_model_ml(ticker, models, window=60, test_period_days=63):
    ...
    plt.plot(dates, predictions_array, label="Predicted % Move", color='#203123')  # Dark green
    plt.plot(dates, actuals_array, label="Actual % Move", color='#FFC87A')  # Light grey
    plt.xlabel("Date")
...
def index():
    if request.method == "POST":
        ...
        # You can either redirect or directly post the ticker value to /info
        return render_template("info.html", ticker=ticker)
...
def info():
    ...
    try:
        ...
        equal_pred = predict_next_day_close(ticker)
    except Exception as e:
        equal_pred = {"error": str(e)}

    try:
        # Train ML model (this might take a while)
        models = train_ml_model(ticker, window=60)
    except Exception as e:
        return f"Error during ML training: {e}"
    
    try:
        ml_pred = predict_next_day_close_ml(ticker, models)
    except Exception as e:
        ml_pred = {"error": str(e)}
    
    try:
        backtest_results = backtest_model_ml(ticker, models, window=60, test_period_days=63)
    except Exception as e:
        backtest_results = {"error": str(e)}
    
    # Compile all results into a dictionary to pass to the template
    results = {
        "ticker": ticker,
        "equal_prediction": equal_pred,
        "ml_prediction": ml_pred,
        "backtest": backtest_results,
        "model_metrics": models.get("model_metrics", {}) if models else {}
    }
