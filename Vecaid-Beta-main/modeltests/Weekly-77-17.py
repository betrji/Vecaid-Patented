...
###############################################################################
# DATA HELPER FUNCTIONS (daily and weekly only)
###############################################################################
...
###############################################################################
# ATR and ADX Functions (default period remains 14 in these low-level funcs)
###############################################################################
...
def adx(data, period=14):
    ...
    atr_val = tr['tr'].rolling(window=period).mean()
    up_move = high.diff()
    down_move = low.diff().abs()
    plus_dm = np.where((up_move > down_move) & (up_move > 0), up_move, 0)
    minus_dm = np.where((down_move > up_move) & (down_move > 0), down_move, 0)
    plus_di = 100 * (pd.Series(plus_dm).rolling(window=period).sum() / atr_val)
    minus_di = 100 * (pd.Series(minus_dm).rolling(window=period).sum() / atr_val)
...
###############################################################################
# BOLLINGER, MFI, Force Index, etc.
###############################################################################
def bollinger_bands(data, period=10, num_std=2):  # updated period
...
def compute_mfi(data, period=10):  # updated period
...
###############################################################################
# TECHNICAL INDICATORS (optimized for a week‐ahead horizon)
###############################################################################
def stochastic_oscillator(data, k_period=10, d_period=3):  # updated period
...
def chaikin_money_flow(data, period=10):
...
def commodity_channel_index(data, period=10):
...
def fwma(data, period=10):
    weights = np.arange(1, period+1)
    def weighted_mean(x):
        if len(x) < period:
            return np.nan
        return np.dot(x, weights) / weights.sum()
    data['FWMA'] = data['Close'].rolling(window=period).apply(weighted_mean, raw=True)
    return data

def tema(data, period=10):
...
def get_technical_indicators(data):
    ...
    data = bollinger_bands(data, period=10, num_std=2)
    data = tema(data, period=10)
    data = fwma(data, period=10)

    data['std'] = data['Close'].rolling(window=10).std()
    delta = data['Close'].diff()
    up = delta.clip(lower=0)
    down = -delta.clip(upper=0)
    roll_up = up.rolling(10).mean()
    roll_down = down.rolling(10).mean()
    ...
    data = stochastic_oscillator(data, k_period=10, d_period=3)
    data = adx(data, period=10)
    data['ROC'] = data['Close'].pct_change() * 100
    data['CMF'] = chaikin_money_flow(data, period=10)
    data['CCI'] = commodity_channel_index(data, period=10)
    ...
    data['Donchian_High'] = data['High'].rolling(window=10).max()
    data['Donchian_Low'] = data['Low'].rolling(window=10).min()
    data['Williams_R'] = -100 * ((data['High'].rolling(window=10).max() - data['Close']) /
                                 (data['High'].rolling(window=10).max() - data['Low'].rolling(window=10).min()))
    data['ATR'] = compute_atr(data, period=10)
    data['MFI'] = compute_mfi(data, period=10)
...
###############################################################################
# BASIC PREDICTION FUNCTION (Week-Ahead)
###############################################################################
def predict_next_day_close(ticker):
    ...
    predicted_date = data.index[-1] + BDay(5)
    return {
...
def train_ml_model(ticker, window=60):
    ...
    if daily_data.empty:
        raise ValueError(f"No daily data found for {ticker} from {start_date} to {split_date}.")
    ...
    # Removed intraday data merge (not used)
    features = []
    targets = []
    print("Creating feature vectors and target values (week-ahead)...")
    # Loop until len(data) - 5 so that we can use the close 5 days ahead
    for i in range(window, len(data) - 5):
        window_data = data.iloc[i-window:i]
        latest_window = window_data.iloc[-1]
        current_price = latest_window['Close']

        # 17-length feature vector (removed intraday_range)
        ...
        next_week_close = data.iloc[i+5]['Close']
        pct_change = ((next_week_close - current_price) / current_price) * 100
    ...
    gru_model.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)

    cnn_lstm_model = build_cnn_lstm_model(input_dim)
    cnn_lstm_model.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)

    cnn_lstm_variant = build_cnn_lstm_variant_model(input_dim)
    cnn_lstm_variant.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)

    bilstm_model = build_bidirectional_lstm_model(input_dim)
    bilstm_model.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)

    dnn_model = build_dnn_model(input_dim)
    dnn_model.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)

    transformer_model = build_transformer_model(input_dim)
    transformer_model.fit(X_pca, y, epochs=200, batch_size=32, verbose=1, validation_split=0.2)
    ...
    xgb_models = [xgb_model_1, xgb_model_2, xgb_model_3, xgb_model_4, xgb_model_5]
    xgb_ensemble_preds = np.mean([m.predict(X_pca).reshape(-1,1) for m in xgb_models], axis=0)
    rmse_xgb = np.sqrt(mean_squared_error(y, xgb_ensemble_preds))

    rmse_gru = np.sqrt(mean_squared_error(y, gru_model.predict(X_pca)))
    rmse_cnn_lstm = np.sqrt(mean_squared_error(y, cnn_lstm_model.predict(X_pca)))
    rmse_cnn_lstm_variant = np.sqrt(mean_squared_error(y, cnn_lstm_variant.predict(X_pca)))
    ...
    rmse_transformer = np.sqrt(mean_squared_error(y, transformer_model.predict(X_pca)))
    ...
    meta_model.fit(X_meta_train, y_meta_train, eval_set=[(X_meta_val, y_meta_val)], verbose=True)
    rmse_meta = np.sqrt(mean_squared_error(y_meta_val, meta_model.predict(X_meta_val)))
...
###############################################################################
# PREDICT FINAL (Week-Ahead, without intraday data)
###############################################################################
def predict_next_day_close_ml(ticker, models):
    ...
    feature_vector = np.array([[tech_sig, cmf, cci,
                                stochastic_sig, adx_sig, roc_sig, bb_sig,
                                rsi_sig, macd_sig, sma_cross_sig, obv_sig,
                                volume_sig, momentum_sig, hlc3_val, tema_val, fwma_val, bb_width_val]])
    ...
    combined = np.concatenate([p_gru, p_cnn_lstm, p_cnn_lstm_variant, p_bilstm, p_dnn, p_trans, p_xgb], axis=1)
...
###############################################################################
# BACKTEST (Week-Ahead Prediction)
###############################################################################
def backtest_model_ml(ticker, models, window=60, test_period_days=63):
    ...
    if data.empty:
        raise ValueError(f"No data found for {ticker} in last {total_days} days.")
    if isinstance(data.columns, pd.MultiIndex):
        if ticker in data.columns.levels[0]:
            data = data[ticker]
        else:
    ...
    print("Beginning backtest loop over", len(data) - window - 5, "days...")
    for i in range(window, len(data) - 5):
        window_data = data.iloc[i-window:i]
        latest_window = window_data.iloc[-1]
        current_price = latest_window['Close']
        ...
        next_week_close = data.iloc[i+5]['Close']
        actual_pct_move = ((next_week_close - current_price) / current_price) * 100
        actual_moves.append(actual_pct_move)
        actual_prices.append(next_week_close)
        ...
        if (i - window) % 10 == 0:
            print(f"Day {data.index[i+5]}: Current Price={current_price:.2f}, Predicted Move={final_pred:.2f}%")
...
if __name__ == "__main__":
    ...
    try:
        # Quick example prediction with the simpler function
    ...
    print("\nTraining final hybrid ensemble ML model...")
    ticker_ml = ticker
    try:
    ...
    if models is not None:
        ...
        print("\nBacktesting the final hybrid ensemble model on the last 3 months...")
        try:
